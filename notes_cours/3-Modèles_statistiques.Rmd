...
En R, la commande suivante génère dix valeurs aléatoires (`n = 10`) tirées de la distribution uniforme (continue) entre 0 et 6, que nous avons vu dans la section précédente.
...
On observe une meilleure correspondance que pour les données non-transformées, sauf au niveau des valeurs les plus petites du DHP dans l'échantillon, qui demeurent plus élevées que prévus par la distribution de référence. Avez-vous une hypothèse pour cette anomalie près du minimum? (Indice: Quels arbres ne sont pas échantillonnés?)
