...
- Perform ordination of community data with correspondence analysis and non-metric multidimensional positioning (NMDS).
...
According to Legendre and Legendre (2012), the Hellinger distance responds more linearly to the changes in abundance of a species. The Hellinger transformation is therefore recommended if we want to carry out a PCA on abundance data. We will see in the next section another ordination method for this type of data, correspondence analysis.
...
- the Bray-Curtis distance if the absolute differences of abundance (number of individuals) are important;
- the Hellinger distance if we are concerned about the relative differences in abundance (proportion of individuals) and we want to give more emphasis to common species;
...
In PCA, the variables were represented by vectors (arrows) and we interpreted the direction and length of these vectors. Here, species and sites are points on the same scale; the proximity of a site to a species indicates an association between the two. Also, the proximity between two sites indicates that they have a similar community and the proximity between two species indicates that they occupy similar sites. The coordinates of species and sites can be extracted with the `scores (ac_dune)` function.

We can see the importance given to rare species granted in this ordination. For example, the species *Chenalbu* at the bottom of the graph has only one individual, which is found at site 13.

In general, the abundance of a species is not a linear function of an environmental gradient (e.g., temperature); the species reaches a maximum at an optimum value and its abundance decreases for larger or smaller values of the environmental variable. Due to this non-linearity, a series of sites ordered according to an environmental gradient may follow a curve on the ordination plot. This could be the case here, with the presence of a spiral starting around the point (0, 0). Methods have been proposed to rectify these curves (detrended correspondence analysis or DCA), but these methods are not recommended, except in the rare cases where we know the ordination is dominated by a single large gradient. A better solution would be to superimpose the environmental gradient on the graph of ordination, as we will see later.

## Non-metric multidimensional scaling

Another ordination method commonly applied in ecology is non-metric multidimensional scaling (NMDS). This method uses a distance matrix between sites as a starting point, then applies an iterative algorithm to find a point configuration in a small number of dimensions (usually 2) that best represents the distance between sites.

The correspondence between the original distance matrix $d$ and the Euclidean distance in the reduced space $\hat{d}$ is measured by the *stress*:

$$ \sqrt{\frac{\sum_{(i, j)} (d_{ij} - \hat{d_{ij}})^2}{\sum_{(i, j)} d_{ij}^2}} $$

In this formula, each sum is calculated over all pairs of observations.

The main advantage of this method is its flexibility. NMDS can be performed according to any measure of multivariate distance. However, it is a heuristic algorithm that may not converge, or converge to different solutions depending on the starting conditions.

The `metaMDS` function addresses the second issue by performing repeated runs of the algorithm from random initial configurations, in order to increase the chances of detecting the optimal result. The Bray-Curtis distance is often recommended for analyzing abundance data with this algorithm and is the default distance choice for `metaMDS`.
...
The `plot` function displays the result of NMDS in two dimensions. After NMDS orders the sites, the position of the species is determined according to the position of the sites where these species are found. We specify the argument `type = "t"` to display the names of sites and species rather than points alone.

```{r}
plot(md_dune, type = "t")
```

Note that this graph is very similar to the results of the correspondence analysis. Compared to the latter, the position of the sites is less influenced by rare species; for example, Site 13 is closer to the center and farther away from the *Chenalbu* species.
...
So far, we have seen how to represent the variation between ecological communities present at various sites. This last section deals with methods for associating this variation with predictors measured for the same sites.
...
If the principal components are orthogonal (which is the case for PCA and CA, but not NMDS), a separate regression of each component can be performed according to the predictors. Another option is to simultaneously estimate the correlation between explanatory variables and each of the axes, which is done by the `envfit` function of *vegan*.

The `envfit` function requires two parameters, the result of an ordination and a data frame for the predictors. Here, we consider the relation between the NMDS components of `dune`, the A1 soil thickness (*A1*) and the type of managament (*Management*).
...
For comparison purposes, here is the biplot of the coresponding unconstrained ordination (correspondence analysis) with the estimate of predictor effects obtained by `envfit`:
...
- Euclidean distance is not appropriate for ecological community data. We have presented in this class several distances designed for abundance data (Bray-Curtis, Hellinger, $\chi^2$) or presence-absence data (Jaccard, Sorenson). The distance matrices obtained by these methods can be used in a cluster analysis or in some ordinations (e.g. NMDS).

- Correspondence analysis is a type of ordination similar to principal component analysis, but adapted to contingency tables, including abundance or presence data.

- Nonmetric multidimensional scaling (NMDS) is an ordination method that optimizes the position of points in a small number of dimensions so that their distance is as close as possible to a given distance matrix.

- The `envfit` function makes it possible to simultaneously estimate the effect of predictors on the main ordination axes.

- Four of the ordination methods seen in this course (PCA, CA, RDA and CCA) are related by two properties: the type of data and the presence or absence of constraints. In constrained ordination, a regression is performed on the multivariate data and ordination is performed on the part of the response explained by the predictors.

|  | Unconstrained | Constrained |
|--+-----------------+-----------------|
| Continuous data (including transformed abundance) | Principal component analysis | Redundancy analysis |
| Contingency table (abundance or presence) | Correspondence analysis | Canonical correspondence analysis |
