...
- Utiliser la régression logistique pour modéliser une réponse binaire, une réponse binomiale (nombre de succès et d'échecs) ou des proportions.
...
Imaginons $n$ répétitions indépendantes d'une expérience qui peut se conclure par un succès (1) ou un échec (0), avec une probabilité de succès $p$ pour chaque répétition. Dans ce cas, le nombre de succès $y$ suit une distribution binomiale: $y \sim B(n, p)$. Selon cette distribution, la probabilité d'obtenir $y$ succès et $n-y$ échecs correspond à:

$$ \frac{n!}{y!(n-y)!} p^y (1-p)^{n-y}  = \binom{n}{y} p^y(1-p)^{n-y}$$

Pour comprendre cette formule, prenons par exemple la séquence de succès et d'échecs 01001 ($n = 5$ et $y = 2$). La probabilité d'obtenir 0 au premier essai est $1-p$, la probabilité d'obtenir 1 au deuxième essai est $p$, et ainsi de suite. La probabilité d'une séquence d'essais indépendants est le produit des probabilités de chaque essai, donc $p^2(1-p)^3$, ce qui correspond à la partie $p^y(1-p)^{n-y}$ de la formule. Toutefois, il s'agit de la probabilité d'une seule séquence de 2 succès et 3 échecs. Chacune des autres séquences possibles (11000, 00110, etc.) a la même probabilité. Ce nombre de séquences possibles est égal à $n!/y!(n-y)!$, où $!$ représente l'opération factorielle.

Le *nombre* moyen de succès est égal à $np$, avec une variance égale à $np(1-p)$. La *proportion* moyenne de succès est égale à $p$ avec une variance de $p(1-p)/n$.
...
Dans R, les fonctions permettant de calculer des quantités à partir de la distribution binomiale portent le suffixe `binom`. Par exemple, la probabilité d'obtenir 2 succès sur 5 essais, avec une probabilité de succès de 0.3, est égale à:
...
Pour relier la probabilité de succès $p$ de la distribution binomiale au prédicteur linéaire $\eta = \beta_0 + \sum_{i = 1}^m \beta_i x_i$, le lien le plus souvent utilisé est la fonction logit.
...
- La pente de la courbe est plus prononcée au centre et plus faible (tendant vers 0) aux extrémités. Du point de vue de la régression, cela signifie que l'effet des variables explicatives (contenues dans $\eta$) sur la probabilité de succès est plus important lorsque celle probabilité est près de 50%. 
...
L'ordonnée à l'origine $\beta_0$ est égale au logit de $p$ lorsque $x = 0$. Pour calculer la valeur de $p$ correspondante, on peut définir la fonction inverse du logit (`invlogit`).
...
Dans R, la régression logistique peut être utilisée pour trois types de réponses:
...
Nous verrons des exemples des trois cas dans ce cours.
...
Pour visualiser l'intervalle de confiance de ces probabilités, nous devons choisir l'option `se.fit = TRUE` de `predict` pour obtenir les erreurs-types, puis ajouter un ruban correspondant à une région de 1.96 erreur-type de part et d'autre de la valeur moyenne, pour un intervalle de confiance à 95%.
...
Dans cet exemple, nous coderons la mortalité comme étant la résultat "positif" (1). Pour appliquer une régression logistique binomiale à ces données, la réponse de notre formule (à gauche du `~`) doit être composée de deux colonnes (nombre de 1 et nombre de 0) regroupées avec `cbind`. Ici, la variable représentant le nombre de 0 (les survivants) est obtenu en soustrayant `Deaths` de `N`. 
...
Puisque la réponse n'est pas supposée suivre une distribution normale, nous ne nous intéressons pas vraiment au diagramme quantile-quantile. Notez que les résidus utilisés dans ce diagramme ne sont pas les résidus de la réponse ($y - \hat{y}$), mais plutôt les résidus de la déviance `residuals(mod_snails, type = "deviance")`, qui s'approchent davantage de la normalité. Ceux-ci sont calculés à partir de la contribution de chaque observation à la déviance totale, $-2 \log L_k$, où $L_k$ est la vraisemblance de l'observation $k$ prise individuellement.
...
Dans l'exemple précédent, la réponse était basée sur des nombres d'individus (escargots morts et vivants au bout de l'expérience), donc la distribution binomiale pouvait s'appliquer exactement. 

Dans d'autres cas, la réponse est directement exprimée sous forme de proportions continues, plutôt qu'un ensemble de mesures discrètes de présence/absence. Il peut s'agir par exemple de la proportion d'un quadrat recouvert par une mousse, la fraction d'une région brûlée par le feu, ou d'une autre variable comprise entre 0 et 1.

Pour ce type de réponse, on peut tout de même utiliser le lien logit pour relier la probabilité ou proportion moyenne à un prédicteur linéaire. Toutefois, la variance de cette proportion ne correspondra pas nécessairement à celle d'une distribution binomiale. Il faut donc utiliser l'option de famille *quasibinomiale* dans `glm`.

Le modèle quasibinomial n'est pas une distribution statistique à proprement parler. En fait, ce modèle estime les coefficients comme s'il s'agissait d'une régression logistique habituelle, mais estime séparément un paramètre de dispersion $\phi$, de façon à ce que la variance des proportions observées soit $\phi p (1-p)$.

Pour illustrer l'application du modèle quasibinomial, créons une nouvelle variable `prop` représentant le taux de mortalité pour chaque combinaison des prédicteurs du tableau de données `snails`. 
...
Les coefficients estimés sont les mêmes que pour la régression logistique binomiale précédente. Les erreurs-types ont un peu changé, puisque la variance du modèle binomiale était fixe et basée sur le nombre d'individus testés. 

Le paramètre de dispersion n'affecte pas les valeurs des coefficients, seulement leurs erreurs-types. Nous en parlerons davantage au prochain cours dans le contexte de la régression de Poisson.

Une variable continue comprise entre 0 et 1 peut aussi être modélisée avec la distribution *beta*. La régression beta (package *betareg* dans R) n'est cependant pas vue dans ce cours.
