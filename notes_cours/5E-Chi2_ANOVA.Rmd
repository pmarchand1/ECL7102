...
The chi-squared distribution, like the $t$ distribution we used in the previous class, has a parameter for the number of degrees of freedom. The degrees of freedom correspond to the number of independent data used in the calculation of a statistic. Here, the $\chi^2$ is calculated from the deviations between observed and expected frequencies for $k$ categories. However, since the sum of the deviations must equal 0 (because the total of $f_i$ and $\hat{f_i}$ is the same) there are $k - 1$ independent deviations.

$$ \sum_{i=1}^n f_i = \sum_{i=1}^n \hat{f_i} $$, thus $$\sum_{i=1}^n (f_i - \hat{f_i}) = 0$$

Similarly, the estimate of the standard deviation for a sample of size $n$:

$$ s = \sqrt{\frac{1}{n - 1} \sum_{i = 1}^n \left( x_i - \bar{x} \right)^2} $$

has $n - 1$ degrees of freedom, since the sum of the deviations from the mean is zero. The $t$ statistic for a sample, which is based on this $s$ calculation, also has $n - 1$ degrees of freedom.
