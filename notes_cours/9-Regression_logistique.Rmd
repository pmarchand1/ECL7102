...
L'ordonnée à l'origine $\beta_0$ est égale au logit de $p$ lorsque $x = 0$. Pour calculer la valeur de $p$ correspondant à une valeur donnée de $\text{logit}(p)$, on peut utiliser la fonction `plogis` dans R, tandis que la transformation logit elle-même est obtenue par la fonction `qlogis`.
