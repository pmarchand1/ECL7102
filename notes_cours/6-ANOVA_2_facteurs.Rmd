...
Pour représenter les interactions entre les deux facteurs, on ajoute un terme $\gamma_ij$ au modèle d'ANOVA.
...
Dans cette équation, $\bar{y_{ij}}$ est la moyenne des observations qui font partie à la fois du groupe $i$ du facteur A et du groupe $j$ du facteur B. Les résidu du modèle pour une observation $y_{ijk}$ est égal à $y_{ijk} - \bar{y_{ij}}$.

En incluant les interactions, le tableau d'ANOVA à deux facteurs prend la forme suivante:
...
Le nombre de degrés de liberté pour l'interaction est le produit du nombre de degrés de liberté de chaque facteur. L'interaction a sa propre statistique $F$ égale au ratio $MSI/MSE$. L'hypothèse nulle correspond à l'absence d'interactions, c'est-à-dire que les effets des deux facteurs sont additifs et que tous les $\gamma_{ij}$ sont de 0.
...
L'équation suivante décrit un modèle linéaire représentant la relation entre un prédicteur numérique $x$ et une réponse numérique $y$. La valeur de $y$ dépend d'un terme constant ($\beta_0$), d'un terme proportionnel à $x$ ($\beta_1 x$) et d'un résidu aléatoire pour chaque observation ($\epsilon$).
...
- Moyenne générale: $\mu = (\mu_1 + \mu2 + \mu_3)/3 = \beta_0$
...
**Question**: Quelle est la signification des coefficients dans ce tableau? Quel est l'effet du troisième régime (`wheat`)?
