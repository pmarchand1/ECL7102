...
In the example we saw at the beginning of this class, we calculated the $t$ statistic for which we know the theoretical distribution, which allowed us to determine a $p$ value, which is the probability of getting a higher deviation than the one observed if the null hypothesis was true.
...
To do a one-sided test, you need to set the `alternative` argument of `t.test` to `less` or` greater`. In the case where our alternative hypothesis is $\bar{x}_A - \bar{x}_B < 0$, we specify `alternative = "less"`.

```{r}
t.test(count ~ spray, data = insectesAB, alternative = "less")
```
