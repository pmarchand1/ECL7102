...
- La valeur $p$ est la probabilité d'obtenir un écart plus élevé que celui observé si l'hypothèse nulle était vraie. 
...
- Par défaut, `var.equal = FALSE`. Dans le doute, mieux vaut supposer que les variables sont différentes.
...
Dans le cas où notre hypothèse alternative est $\bar{x}_A - \bar{x}_B < 0$, on spécifie `alternative = "less"`.
