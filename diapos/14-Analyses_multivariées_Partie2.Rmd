...
.small[
Voici un dendrogramme créé à partir d'une matrice de distance donnée avec la méthode du saut maximum (`method = "complete"`).

Quelle est la signification de la hauteur "Height" à laquelle deux groupes se joignent? À quelle hauteur environ faudrait-il couper l'arbre pour obtenir 5 groupes?
]

```{r echo=FALSE,message=FALSE, fig.dim = c(7,5)}
library(vegan)
data(dune)
dat <- decostand(dune, "hellinger")
hcl <- hclust(dist(dat), method = "complete")
plot(hcl)
```
...
- La PCoA transforme la matrice de distance pour produire des axes principaux, tandis que le NMDS déplace les points dans un nombre de dimensions choisis au hasard afin de s'approcher le plus possible des distances réelles.
...
- `decostand`: Calcule une tranformation des données d'une matrice.
...
- `cca` Effectue une analyse de correspondance (si appelée sans prédicteurs) ou une analyse caononique des correspondances.
