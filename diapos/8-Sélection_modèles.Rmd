...
knitr::opts_chunk$set(echo = TRUE, fig.dim = c(8, 6))
library(dplyr)
library(ggplot2)
library(cowplot)
```
...
Voici les coefficients d'une régression de la masse foliaire $F$ (en grammes) d'une espèce de plantes mesurée sur plusieurs placettes en fonction de la température moyenne $T$ (en Celsius) et de la densité $D$ (en plantes/m<sup>2</sup>). 
...
(a) Quelle est l'équation correspondant à ce modèle? 

(b) Selon ce modèle, la température a-t-elle un effet positif ou négatif sur la masse foliaire, pour une placette avec 1 plante/m<sup>2</sup>? Est-ce la même chose pour une placette avec 10 plantes/m<sup>2</sup>?
...
- Deux versions rapportés dans le résultat de `lm`: *Multiple R-squared* et *Adjusted R-squared*. 
