...
gr_puis <- ggplot(NULL, aes(x = seq(-3, 5, 0.1))) +
    labs(x = "z", y = "p(z)") +
    stat_function(fun = dnorm) +
    stat_function(fun = function(x) ifelse(abs(x) > qnorm(0.975), dnorm(x), NA),
                  geom = "area", fill = "red") +
...
b) Plus le seuil de signification $\alpha$ est petit, plus il est difficile de rejeter l'hypothèse nulle.
...
a) La valeur $p$ est la probabilité que l'hypothèse nulle soit vraie. **Faux, la valeur *p* est la probabilité d'avoir observé une valeur plus extrême de la statistique du test *si* l'hypothèse nulle est vraie.**

b) Plus le seuil de signification $\alpha$ est petit, plus il est difficile de rejeter l'hypothèse nulle. **Vrai.**

c) Pour un test $t$ à un échantillon, si on rejette l'hypothèse nulle $\mu = 10$ avec un seuil $\alpha = 0.01$, alors l'intervalle de confiance à 99% de $\bar{x}$ ne contient pas la valeur 10. **Vrai.**

d) Pour le test $t$ comparant deux échantillons avec l'hypothèse nulle $\mu_A = \mu_B$, une valeur $p$ plus petite signifie que la différence entre les moyennes est plus élevée. **Faux, cela dépend aussi de la variabilité des données et de la taille de l'échantillon.**
...
library(dplyr)
insectesAB <- InsectSprays[InsectSprays$spray %in% c("A", "B"), ]
