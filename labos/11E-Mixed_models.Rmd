...
(c) From appropriate diagnostic plots, check if the mixed model residuals in (b) show no residual pattern and follow a normal distribution.
...
(b) Now fit a mixed model that takes into account that these are repeated measurements on the same trees (*Note*: Ignore the plot for now.) and answer the following questions: (i) How do you interpret the standard deviation of the random effect and the residual standard deviation? (ii) Based on these results, why is it beneficial to measure the same trees on each sampling day to estimate a growth curve?
