...
Answers for this lab must be submitted before **November 7th at 5pm on Moodle**. In your answer for each question, please include a copy of the R code used (if applicable) and the results obtained.
...
b) Taking into account your result in (a), propose several alternative models for this dataset, including the null model (0 predictor) and models with 1 or 2 predictors (without interactions). Create a table comparing these models according to their AICc.
...
- Latitude of the wintering site in Africa (*latWntr*). *Note*: Latitude is positive north of the equator.
...
How many models have a $\Delta AIC \le 2$? According to the Akaike weights, what is the probability that one of these models is the best model?
